        ...
        <div className="border-t border-slate-700 pt-8">
          ...
          <div className="space-y-4">
            {education.map((edu, idx) => (
              <div key={idx} className="p-4 bg-slate-900 rounded-xl border border-slate-700 flex gap-4 items-start relative pb-6 md:pb-4">
                <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                  <input 
                    placeholder="Degree (e.g. B.S. in CS)" 
                    value={edu.degree} 
                    onChange={e => updateEducation(idx, 'degree', e.target.value)}
                    className="bg-slate-800 border-none rounded-lg px-3 py-2 text-white w-full"
                  />
                  <input 
                  ...
                  <input 
                    ...
                    className="bg-slate-800 border-none rounded-lg px-3 py-2 text-white w-full md:col-span-2"
                  />
                </div>
                <button type="button" onClick={() => removeEducation(idx)} className="text-red-400 hover:bg-red-400/10 p-2 rounded-lg absolute bottom-[10px] right-[10px] md:static">
                  <Trash2 size={18}/>
                </button>
              </div>
            ))}
        ...
        <div className="border-t border-slate-700 pt-8">
          ...
          <div className="space-y-4">
            {languages.map((lang, idx) => (
              <div key={idx} className="p-4 bg-slate-900 rounded-xl border border-slate-700 flex gap-4 items-center">
                <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                  <input 
        ...
        <div className="border-t border-slate-700 pt-8">
          ...
          <div className="space-y-4">
            {experiences.map((exp, idx) => (
              <div key={idx} className="p-4 bg-slate-900 rounded-xl border border-slate-700 flex gap-4 items-start relative pb-8 md:pb-4">
                <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                  <input 
                ...
                <button type="button" onClick={() => removeExperience(idx)} className="text-red-400 hover:bg-red-400/10 p-2 rounded-lg py-1 absolute bottom-[10px] right-[10px] md:static">
                  <Trash2 size={18}/>
                </button>
              </div>
            ))}
